...
class ContextMenuBridge<Type> {
  ...
  hookListeners: Array<
    (event: CMMouseEvent, data: Type, open: boolean) => void
  > = [];
  ...
  dispatch = (event: CMMouseEvent) => {
    this.hookListeners.forEach((handler) => {
      handler(event, this.data, this.open);
    });
...
const createBridge: <Type>(defaultData: Type) => ContextMenuBridge<Type> = (
  data
